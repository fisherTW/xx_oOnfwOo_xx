		...
		<script src='<?= base_url()?>assets/js/hire_factory_list.js'></script>
...
$(function () {
	$("#btn_create").bind("click",function(){
		window.location = '<?= base_url()?>bos/hire_factory_edit';
	});	

	$('#tblmain').bootstrapTable({
		toggle:"table",
		idField: 'id',
		url: '<?= base_url()?>bos/apiGetHire_factory',
		formatSearch:function(){
			return '<?= $lang->line('search').' '.$lang->line('employed_workers_id') ?>';
		},
		sortName:"id",
		sortOrder:"desc",
...
function operateFormatter(value, row, index) {
		...
		if((row.f_file_path2 != null) && (row.f_file_path2.length > 0)) {
			ary_ret.push(
				'　',
				'<a class="mdl2 ml10" href="javascript:void(0)" title="<?= $lang->line('f_file_path2') ?>">',
					'<i class="glyphicon glyphicon-cloud-download"></i>',
				'</a>'
			);
		} else {
		...
		if((row.f_w_file_path != null) && (row.f_w_file_path.length > 0)) {
			ary_ret.push(
				'　',
				'<a class="mdl3 ml10" href="javascript:void(0)" title="<?= $lang->line('f_w_file_path') ?>">',
					'<i class="glyphicon glyphicon-cloud-download"></i>',
				'</a>'
			);
		} else {
