...
<?php endif; ?>				
		...
		<div class="form-group">
			<label for="inputPassword3" class="col-sm-2 control-label"><?= $lang->line('emp_position_category') ?></label>
			<div class="col-sm-10">
<?php if (strval($id) != '0'): ?>
				<?= form_dropdown('sel_emp_position_category_id', $ary_emp_position_category_id, $info['emp_position_category_id'], 'class="form-control"'); ?>
<?php else: ?>
				<?= form_dropdown('sel_emp_position_category_id', $ary_emp_position_category_id, '', 'class="form-control"'); ?>
<?php endif; ?>				
			</div>
		</div>
		<div class="form-group">
...
$(function () {
	$(".btn-cancel").bind("click",function(){
		window.location = '<?= base_url()?>config/emp_position';
	});	
	$("#btn_save").bind("click",function(){
		...
		$.ajax({
			type: "POST",
			url: $('#hid_baseurl').val() + 'config/emp_position_doEdit',
