...
<div class="container">
	<form class="form-horizontal" id='form1'>
		...
		<input type='hidden' name='hid_type' value='<?= $type ?>'>

		<h1><?= $title ?></h1>
		<div class="form-group">
			<label for="inputPassword3" class="col-sm-2 control-label"><?= $lang->line('1-'.$zcat.'-'.$ztype).$lang->line('docname') ?></label>
			<div class="col-sm-10">
<?php if (strval($id) != '0'): ?>
...
$(function () {
	$(".btn-cancel").bind("click",function(){
		window.location = '<?= base_url()?>config2/doc/<?= $type?>';
	});	
	$("#btn_save").bind("click",function(){
		...
		$.ajax({
			type: "POST",
			url: $('#hid_baseurl').val() + 'config2/doc_doEdit/0/<?= $type?>',
