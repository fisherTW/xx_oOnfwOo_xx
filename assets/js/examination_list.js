function operateFormatter(value, row, index) {
	...
	if((row.file_path2 != null) && (row.file_path2.length > 0)) {
		ary_ret.push(
			'　',
			'<a class="mdl2 ml10" href="javascript:void(0)" title="download2">',
				'<i class="glyphicon glyphicon-cloud-download"></i>',
			'</a>'
		);
	} else {
...
window.operateEvents = {
	'click .medit': function (e, value, row, index) {
		window.location = $('#baseUrl').val() + 'eam/examination_edit/' + row.id ;
	},
	'click .mremove': function (e, value, row, index) {
		$.ajax({
			type: "POST",
			url: $('#baseUrl').val() + 'eam/examination_doDel/' + row.id,
			data: { id: row.id },
			statusCode: {
				200: function() {
					alert("成功刪除");
