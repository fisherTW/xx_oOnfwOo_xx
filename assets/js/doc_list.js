function operateFormatter(value, row, index) {
	if(((row.id.toString() == '1') || (row.id.toString() == '2')) && (row.type.toString() == '5.1')) {
		return '';
	} else {
		return [
...
window.operateEvents = {
	'click .medit': function (e, value, row, index) {
		window.location = $('#baseUrl').val() + 'config2/doc_edit/' + row.id + '/' + row.type;
	},
	'click .mremove': function (e, value, row, index) {
		$.ajax({
			type: "POST",
			url: $('#baseUrl').val() + 'config2/doc_doDel/' + row.id,
			data: { id: row.id },
			statusCode: {
				200: function() {
					alert("成功刪除");
