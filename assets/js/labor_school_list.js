...
window.operateEvents = {
	'click .medit': function (e, value, row, index) {
		window.location = $('#baseUrl').val() + 'eam/labor_school_edit/' + row.school_id + '_' + row.schoolclass_id ;
	},
	'click .mremove': function (e, value, row, index) {
		$.ajax({
			type: "POST",
			url: $('#baseUrl').val() + 'eam/labor_school_doDel/' + row.school_id + '_' + row.schoolclass_id,
			data: {},
			statusCode: {
				200: function() {
					alert("成功刪除");
